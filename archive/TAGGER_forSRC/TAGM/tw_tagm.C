...
// Oct 2021 TJH
// Added min_evt argument
// There are small "reflections" that can screw up the fit
// By throwing away slices that have too few events, this can be gotten under control
// The mechanism was there already, but this puts it as a command line argument for quicker adjustments
// I have found ~100 events per evio file used to be a decent starting guess for the cutoff
// 
// The other thing to look at for getting the fit right is the initial value of parameter 3 in the timewalk fit
// That, plus the bounds seems to have a big effect  on the success of the fitting
// For a given counter, it seems to fail if the starting value is much larger than the spread in "x" of the timewalk data to be fit
// (e.g. if it goes from 100-400, a starting value of 500 will likely fail)
// This could probably be more methodically set, but I haven't written that yet
void tw_tagm(TString name, int run_numb, int min_evt = 500,int ch_min = 1, int ch_max = 102, int debug_counter_fit = 0){
  ...
  TFile *file = new TFile (name.Data(),"R"); 
  ...
  sprintf(out_file,"tagm_tw_new_%d.txt",run_numb);
  ...
  c1->Divide(1,2);
  //c1->Print("timewalk_fit_slices.pdf(", "pdf");

  TCanvas *c2 = new TCanvas("c2","c2", 700, 10, 450, 450);
  c2->Print("timewalk_fit.pdf(", "pdf");
  ...
  for(int ch = ch_min; ch <= ch_max; ch++){
    ...
    for(int slice = 0; slice < 125; slice++){
      ...
      // Add height minimum to try to cut out reflections
      if(f1->GetParameter(1) < 2. && f1->GetParameter(1) > -1. && f1->GetParameter(0)>100){
        ...
        ey[npoint] = f1->GetParError(1)/f1->GetParameter(0); //Add in the height to the uncertainty to make it more likely the main band gets fit
    ...
    func5->SetParameter(1,30);
    func5->SetParameter(2,0.6);
    func5->SetParameter(3,-100);
    func5->SetParLimits(3, -600, 0);
